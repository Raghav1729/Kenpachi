...
final class CacheManager {
  ...
  /// Timer to periodically purge caches
  ...
  private init() {
    ...
    // Schedule periodic cache purge every 60 seconds
    ...
    timer.setEventHandler { [weak self] in
      ...
      self.clearDiskCache()
      AppLogger.shared.log("Scheduled cache purge executed", level: .debug)
    }
    timer.resume()
  ...
  func cacheInMemory(
    data: Data, forKey key: String, expirationTime: TimeInterval = AppConstants.Cache.expirationTime
  ) {
